...
void icmpv6rpl_writeDODAGid(uint8_t *dodagid) {
    ...
    memcpy(&(icmpv6rpl_vars.dio.DODAGID[0]), dodagid, sizeof(icmpv6rpl_vars.dio.DODAGID));

    memcpy(&(icmpv6rpl_vars.dao.DODAGID[0]), dodagid, sizeof(icmpv6rpl_vars.dao.DODAGID));
        ...
        if (neighbors_isStableNeighborByIndex(i)) { // in use and link is stable
            ...
            if (
                    icmpv6rpl_vars.lowestRankInHistory<(MAXDAGRANK - DAGMAXRANKINCREASE) &&
                                                       tentativeDAGrank>(
                            icmpv6rpl_vars.lowestRankInHistory + DAGMAXRANKINCREASE)
                    ) {
                continue;
            }
    ...
    while (optionsLen > 0) {
        switch (current[0]) {
            case RPL_OPTION_CONFIG:
                ...
                icmpv6rpl_vars.incomingConf->maxRankIncrease = (icmpv6rpl_vars.incomingConf->maxRankIncrease << 8) |
                                                               (icmpv6rpl_vars.incomingConf->maxRankIncrease
                                                                       >> 8); //  2048
                icmpv6rpl_vars.incomingConf->minHopRankIncrease =
                        (icmpv6rpl_vars.incomingConf->minHopRankIncrease << 8) |
                        (icmpv6rpl_vars.incomingConf->minHopRankIncrease >> 8); //256
                icmpv6rpl_vars.incomingConf->OCP =
                        (icmpv6rpl_vars.incomingConf->OCP << 8) | (icmpv6rpl_vars.incomingConf->OCP >> 8); // 0 OF0
                icmpv6rpl_vars.incomingConf->lifetimeUnit = (icmpv6rpl_vars.incomingConf->lifetimeUnit << 8) |
                                                            (icmpv6rpl_vars.incomingConf->lifetimeUnit >> 8); // 0xffff
...
void sendDIO(void) {
    ...
    ((icmpv6rpl_config_ht *) (msg->payload))->minHopRankIncrease =
            (icmpv6rpl_vars.conf.minHopRankIncrease << 8) | (icmpv6rpl_vars.conf.minHopRankIncrease >> 8); //256
    ((icmpv6rpl_config_ht *) (msg->payload))->OCP =
            (icmpv6rpl_vars.conf.OCP << 8) | (icmpv6rpl_vars.conf.OCP >> 8); // 0 OF0
    ((icmpv6rpl_config_ht *) (msg->payload))->lifetimeUnit =
            (icmpv6rpl_vars.conf.lifetimeUnit << 8) | (icmpv6rpl_vars.conf.lifetimeUnit >> 8); // 0xffff
    ...
    */
    ...
    for (nbrIdx = 0; nbrIdx < MAXNUMNEIGHBORS; nbrIdx++) {
        if ((neighbors_isNeighborWithHigherDAGrank(nbrIdx)) == TRUE) {
            ...
            icmpv6rpl_vars.dao_target.optionLength =
                    LENGTH_ADDR128b + sizeof(icmpv6rpl_dao_target_ht) - 2; //no header type and length
            icmpv6rpl_vars.dao_target.type = OPTION_TARGET_INFORMATION_TYPE;
