...
enum {
   ...
   ERR_SENDING_CLIENT_HELLO            = 0x63, // [BLUE]sending client hello message, state: {0}, next state in: {1}[END]
   ERR_PARSING_SERVER_HELLO            = 0x64, // [BLUE]parsing server hello message, state: {0}, next state in: {1}[END]
   ERR_PARSING_SERVER_CERT             = 0x65, // [BLUE]processing server certificate, state: {0}, next state in: {1}[END]
   ERR_PARSING_SERVER_KEX              = 0x66, // [BLUE]processing server key exchange message, state: {0}, next state in: {1}[END]
   ERR_CERTIFICATE_REQUEST             = 0x67, // [BLUE]process possible certificate request server, state: {0}, next state in: {1}[END]
   ERR_TLS_HANDSHAKE_FAILED            = 0x68, // [RED]TLS handshake failed with error code {0} in state {1}[END]
   ERR_TLS_TRUSTED_CERT                = 0x69, // [MAGENTA]skip trusted certificate[END]
   ERR_TLS_STATE_DONE                  = 0x6a, // [BG-GREEN][BOLD][WHITE]DONE![END]
   ERR_PARSING_SERVER_HELLO_DONE       = 0x6b, // [BLUE]processing server hello done, state: {0}, next state in: {1}[END]
   ERR_PREP_CLIENT_CERT                = 0x6c, // [BLUE]preparing possible client cert, state: {0}, next state in: {1}[END]
   ERR_SENDING_CLIENT_KEX              = 0x6d, // [BLUE]sending client key exchange, state: {0}, next state in: {1}[END]
   ERR_MBEDTLS_ERROR                   = 0x6e, // [RED]MBEDTLS failed with error codes: {0} - {1}[END]
   ERR_TCP_RETRANSMISSION              = 0x6f, // retransmission attempt
   ERR_UPDATE_READ_BUFFER              = 0x70, // updating receive buffer, read: {0}, left: {1}
   ERR_CERT_VERIFY                     = 0x71, // [BLUE]possibly verify certificate, state: {0}, next state in: {1}[END]
   ERR_CLIENT_CHANGE_CIPHER_SPEC       = 0x72, // [BLUE]client change cipher spec, state: {0}, next state in: {1}[END]
   ERR_CLIENT_DONE                     = 0x73, // [BLUE]client done, state: {0}, next state in: {1}[END]
   ERR_SERVER_CHANGE_CIPHER_SPEC       = 0x74, // [BLUE]server change cipher spec, state: {0}, next state in: {1}[END]
   ERR_SERVER_DONE                     = 0x75, // [BLUE]server done, state: {0}, next state in: {1}[END]
   ERR_FLUSH_BUFFERS                   = 0x76, // [BLUE]flushing buffers, state: {0}, next state in: {1}[END]
   ERR_HANDSHAKE_WRAPUP                = 0x77, // [BLUE]wrapping up handshake, state: {0}, next state in: {1}[END]
   ERR_WAITING_FOR_DATA                = 0x78, // [YELLOW]waiting for handshake data[END]
   ERR_WAITING_FOR_TX                  = 0x79, // [YELLOW]waiting for transmission of data, state: {0}, next state in: {1}[END]
   ERR_BUSY_IN_STATE                   = 0x7a, // [YELLOW]still processing previous state: {0}, output left: {1}[END]
   ERR_TLS_MEM_ALLOC_FAILED            = 0x7b, // [RED]heap memory allocation failed (no more memory available)[END]
   ERR_UPDATE_DAGRANK                  = 0x7c, // [GREEN]Received DIO and update DAGRANK: {0}[END]
   ERR_SEND_DAO                        = 0x7d, // [GREEN]Sent DAO packet[END]
   ERR_NEW_ENTRY                       = 0x7e, // [GREEN]Added new neighbor: {0} to neighbor table.[END]
   ERR_REMOVE_ENTRY                    = 0x7f, // [YELLOW]Added new neighbor: {0} to neighbor table.[END]
   ERR_BAD_RSSI                        = 0x80, // [RED]Packet discarded, bad rssi: {0}[END]
   ERR_INSUFFICIENT_TX                 = 0x81, // [YELLOW]Don't update MyDagRank after DIO, insufficient TX[END]
   ERR_TX_6LOWPAN_FRAGMENT_FAILED      = 0x82, // [RED]Transmission of 6LowPAN fragment failed[END]
   ERR_ALLOC_NUM_ENTRIES               = 0x83, // [MAGENTA]Allocate openqueue entry, total entries {0}, creator {1} [END]
   ERR_FREE_NUM_ENTRIES                = 0x84, // [MAGENTA]Free openqueue entry, total entries {0}, creator {1} [END]
   ERR_MISSING_FRAGS                   = 0x85, // [RED] Message was declared to be fragmented, but no fragments were found. [END]
   ERR_TECHO_SENT_SUCCESS              = 0x86, // techo data was successfully received by destination
   ERR_TCP_RETRANSMISSION_FAILED       = 0x87, // [RED]retransmission attempt failed[END]
   ERR_REASSEMBLE                      = 0x88, // reassemble 6lowpan fragments, size: {0}
};
